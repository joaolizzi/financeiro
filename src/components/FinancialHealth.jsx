...
export default function FinancialHealth({expenses,income}){
 ...
 return <section className="health-shell panel"><div className="health-head"><div><span className="health-kicker"><ShieldCheck size={13}/> FINANCIAL HEALTH</span><h2>Saúde financeira</h2><p>Um indicador pessoal baseado no equilíbrio do seu mês.</p></div><span className={`health-level score-${data.score>=70?'good':data.score>=55?'mid':'low'}`}>{data.level}</span></div><div className="health-grid"><div className="health-score"><svg viewBox="0 0 120 120"><circle className="health-track" cx="60" cy="60" r="52"/><circle className="health-ring" cx="60" cy="60" r="52" strokeDasharray={circumference} strokeDashoffset={offset}/></svg><div><strong>{data.score}</strong><span>/100</span></div></div><div className="health-copy"><b>{data.summary}</b><span>Este score é apenas um indicador interno do Finanças e não tem relação com score de crédito bancário.</span><div className="health-metrics"><article><Wallet size={15}/><div><small>Renda usada</small><b>{(data.used*100).toFixed(0)}%</b></div></article><article><Activity size={15}/><div><small>Gastos fixos</small><b>{(data.fixedRatio*100).toFixed(0)}%</b></div></article><article><Target size={15}/><div><small>Maior categoria</small><b>{data.top?.[0]||'—'}</b></div></article><article><TrendingUp size={15}/><div><small>Saldo atual</small><b>{money(data.balance)}</b></div></article></div></div></div></section>;
}
