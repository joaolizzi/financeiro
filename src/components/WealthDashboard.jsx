...
export default function WealthDashboard({userId,month,year}){
 ...
 useEffect(()=>{let alive=true;(async()=>{setLoading(true);setError('');const start=shiftMonth(year,month,-11),next=shiftMonth(year,month,1),last=`${next.year}-${String(next.month).padStart(2,'0')}-01`;const[b,c,p,s]=await Promise.all([
   ...
   supabase.from('finance_wealth_snapshots').select('ano,mes,disponivel,guardado,investimentos,ativos,divida_cartoes,patrimonio_liquido,limite_cartoes,captured_at').eq('user_id',userId).gte('ano',start.year).lte('ano',year).order('ano',{ascending:true}).order('mes',{ascending:true})
  ]);if(!alive)return;const err=b.error||c.error||p.error;if(err)setError(err.message);const snapshotsUnavailable=Boolean(s.error);setSnapshotReady(!snapshotsUnavailable);setData({balances:b.data||[],cards:c.data||[],purchases:p.data||[],snapshots:s.data||[]});setLoading(false)})().catch(e=>{if(alive){setError(e.message||'Não foi possível carregar o patrimônio.');setLoading(false)}});return()=>{alive=false}},[userId,month,year]);
 ...
 const calc=useMemo(()=>{
  ...
  const points=data.snapshots.filter(x=>{const n=Number(x.ano)*12+Number(x.mes),min=shiftMonth(year,month,-11);return n>=min.year*12+min.month&&n<=year*12+month}).map(x=>({y:Number(x.ano),m:Number(x.mes),label:monthLabel(Number(x.ano),Number(x.mes)),value:Number(x.patrimonio_liquido||0),real:true}));
  const selectedIndex=points.findIndex(x=>x.y===year&&x.m===month);if(selectedIndex>=0)points[selectedIndex]={...points[selectedIndex],value:net};else points.push({y:year,m:month,label:monthLabel(year,month),value:net,real:false});points.sort((a,b)=>(a.y*12+a.m)-(b.y*12+b.m));
  const history=points.slice(-6),prev=history.at(-2)?.value??net,variation=net-prev,variationPct=Math.abs(prev)>0?variation/Math.abs(prev)*100:0;
