...
export default function FinanceInsights({expenses,income}){
 ...
 return <section className="insights-grid">
  <article className="panel insight-card"><div className="panel-head"><div><h2>Onde seu dinheiro está indo</h2><p>Distribuição dos gastos de {expenses.length?'este mês':'este mês'}.</p></div><PieChart size={20}/></div>{categories.length?<div className="category-bars">{categories.slice(0,6).map(([name,value])=><div className="category-row" key={name}><div><span>{name}</span><b>{money(value)}</b></div><div className="category-track"><i style={{width:`${total?value/total*100:0}%`}}/></div></div>)}</div>:<div className="empty compact">Adicione gastos para ver a distribuição.</div>}</article>
