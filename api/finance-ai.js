...
export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `Você é o assistente financeiro pessoal do aplicativo Finanças. Analise SOMENTE os dados fornecidos abaixo. Não invente valores. Responda em português do Brasil, de forma objetiva e útil. Pode calcular totais, médias, percentuais e comparar meses usando os dados. Não dê recomendações de investimento específicas nem trate sua resposta como aconselhamento financeiro profissional. Se não houver dados suficientes, diga claramente.
...
${question.trim()}`;
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`, {
      ...
      body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }] }], generationConfig: { temperature: 0.2, maxOutputTokens: 700 } })
    });

    const result = await response.json();
    if (!response.ok) return res.status(response.status).json({ error: result?.error?.message || 'Não foi possível consultar a IA.' });
